...
module('Unit | Model | query', function(hooks) {
  ...
  test('attributes (string, regex, boolean, number)', function(assert) {
    ...
    // string
    store.query('post', { filter: { name: 'Super Name' } }).then(function (posts) {
      assert.equal(posts.length, 1);
    });

    // boolean
    store.query('post', { filter: { isPrivate: false } }).then(function (posts) {
      assert.equal(posts.length, 1);
    });
    ...
    // regex
    store.query('post', { filter: { name: /^Just(.*)/ } }).then(function (posts) {
      assert.equal(posts.length, 2);
    });
  ...
  test('belongsTo relationship', function(assert) {
    ...
    // object
    store.query('post', { filter: { user: { id: id } } }).then(function (posts) {
      assert.equal(posts.length, 2);
    });
    ...
    // object regex
    store.query('post', { filter: { user: { id: regexId } } }).then(function (posts) {
      assert.equal(posts.length, 2);
    });

    // polymorphic
    // get posts from editors
    store.query('post', { filter: { user: { type: 'editor' } } }).then(function (posts) {
      assert.equal(posts.length, 3);
    });
    // regex
    store.query('post', { filter: { user: { type: /^ed(.*)ors$/ } } }).then(function (posts) {
      assert.equal(posts.length, 3);
    });

    // get posts from editor '123'
    store.query('post', { filter: { user: { id: id, type: 'editor' } } }).then(function (posts) {
      assert.equal(posts.length, 2, 'nuu');
    });
    // regex
    store.query('post', { filter: { user: { id: id, type: /^ed(.*)ors$/ } } }).then(function (posts) {
      assert.equal(posts.length, 2, 'ups');
      done();
    });
  });
  ...
  test('hasMany relationship', function(assert) {
    ...
    // object
    store.query('user', { filter: { projects: { id: id } } }).then(function (users) {
      assert.equal(users.length, 2);
    });

    // regex
    store.query('user', { filter: { projects: regexId } }).then(function (users) {
      assert.equal(users.length, 2);
    });

    // object regex
    store.query('user', { filter: { projects: { id: regexId } } }).then(function (users) {
      assert.equal(users.length, 2);
    });

    // polymorphic
    // get users with cats
    store.query('user', { filter: { pets: { type: 'cat' } } }).then(function (users) {
      assert.equal(users.length, 2);
    });

    // get users with cat '123'
    store.query('user', { filter: { pets: { id: cat.id, type: 'cat' } } }).then(function (users) {
      assert.equal(users.length, 2);
    });

    // get users with cats AND dogs
    store.query('user', { filter: { pets: [{ type: 'cat' }, { type: 'dog' }] } }).then(function (users) {
      assert.equal(users.length, 1);
    });

    // get users with cats OR dogs
    store.query('user', { filter: { pets: { type: /cats|dogs/ } } }).then(function (users) {
